...
describe('OCA.Files.App tests', function() {
	...
	beforeEach(function() {
		...
		parseUrlQueryStub = sinon.stub(App, '_parseUrlQuery');
	...
	describe('URL handling', function() {
		it('pushes the state to the URL when current app changed directory', function() {
			$('#app-content-files').trigger(new $.Event('directoryChanged', {dir: 'subdir'}));
			expect(pushStateStub.calledOnce).toEqual(true);
			expect(pushStateStub.calledWith({dir: 'subdir', view: 'files'})).toEqual(true);
		});
		describe('onpopstate', function() {
			it('sends "urlChanged" event to current app', function() {
				var handler = sinon.stub();
				$('#app-content-files').on('urlChanged', handler);
				App._onPopState({state: {view: 'files', dir: '/somedir'}});
			...
			it('sends "show" event to current app and sets navigation', function() {
				...
				App._onPopState({state: {view: 'other', dir: '/somedir'}});
			...
			it('parses state from URL when no state passed', function() {
				var handler = sinon.stub();
				parseUrlQueryStub.returns({view: 'other', dir: '/otherdir'});
				$('#app-content-other').on('urlChanged', handler);
				App._onPopState();
				expect(handler.calledOnce).toEqual(true);
				expect(handler.getCall(0).args[0].view).toEqual('other');
				expect(handler.getCall(0).args[0].dir).toEqual('/otherdir');
			});
